...
mvInpTot_ry(comm, region, year)
mvOutTot(comm, region, year, slice)
mvOutTot_ry(comm, region, year)
...
*@ mTechVarom(tech, comm, region, year)
...
*@ mvBalance_ry(comm, region, year)
vBalance_ry(comm, region, year)                     Net commodity balance by region and year (weighted)
;
positive variables
*@ mvOutTot(comm, region, year, slice)
vOutTot(comm, region, year, slice)                   Total commodity output (all processes) (weighted)
*@ mvOutTot_ry(comm, region, year)
vOutTot_ry(comm, region, year)                   Total commodity output (all processes) (weighted)
*@ mvInpTot(comm, region, year, slice)
vInpTot(comm, region, year, slice)                   Total commodity input (all processes) (weighted)
*@ mvInpTot_ry(comm, region, year)
vInpTot_ry(comm, region, year)                   Total commodity input (all processes) (weighted)
*@ mvInp2Lo(comm, region, year, slice, slice)
...
*!eqTechVarom(tech, region, year)  added
;
...
eqBalance_ry(comm, region, year)
eqOutTot(comm, region, year, slice)     Total commodity output
eqOutTot_ry(comm, region, year)     Total commodity output
eqInpTot(comm, region, year, slice)     Total commodity input
eqInpTot_ry(comm, region, year)     Total commodity input
...
eqOutTot_ry(comm, region, year)..
  vOutTot_ry(comm, region, year)
  =e=
  sum(slice,
      pSliceWeight(year, slice) *
      vOutTot(comm, region, year, slice)
...
eqInpTot_ry(comm, region, year)..
        vInpTot_ry(comm, region, year)
        =e=
        sum(slice,
            pSliceWeight(year, slice) *
            vInpTot(comm, region, year, slice)
...
*!        + sum(sup$mvSupCost(sup, region, year), vSupCost(sup, region, year)$mvSupCost(sup, region, year))
         sum((sup, comm, slice)$mSupAva(sup, comm, region, year, slice),
             pSupCost(sup, comm, region, year, slice)
             * pSliceWeight(year, slice)
             * vSupOut(sup, comm, region, year, slice))
* technology capital costs
*!        + sum(tech$mTechEac(tech, region, year), vTechEac(tech, region, year)$mTechEac(tech, region, year))
      + sum((tech, yearp)$(mTechNew(tech, region, yearp)
                    and
                    ordYear(year) >= ordYear(yearp)
                    and
                    (ordYear(year) < pTechOlife(tech, region) + ordYear(yearp)
...
* technology endogenous retirement of pre-built capacity (stock)
        + sum(tech$mvTechRetiredStock(tech, region, year),
              pTechRetCost(tech, region, year) * vTechRetiredStock(tech, region, year))
...
*!        + sum(tech$mTechFixom(tech, region, year), vTechFixom(tech, region, year)$mTechFixom(tech, region, year))
        + sum(tech$mTechFixom(tech, region, year), pTechFixom(tech, region, year) * vTechCap(tech, region, year))
...
*!        + sum(tech$mTechVarom(tech, region, year), vTechVarom(tech, region, year)$mTechVarom(tech, region, year))
        + sum((tech, slice)$mTechSlice(tech, slice),
            pTechVarom(tech, region, year, slice)
            * pSliceWeight(year, slice)
            * vTechAct(tech, region, year, slice)
            +
...
*!        + sum(stg$mStorageEac(stg, region, year), vStorageEac(stg, region, year)$mStorageEac(stg, region, year))
         + sum((stg, yearp)$(mStorageNew(stg, region, yearp)
                    and ordYear(year) >= ordYear(yearp)
                    and (mStorageOlifeInf(stg, region)
                         or ordYear(year) < pStorageOlife(stg, region) + ordYear(yearp)
                         )
...
*!        + sum(stg$mStorageFixom(stg, region, year), vStorageFixom(stg, region, year)$mStorageFixom(stg, region, year))
        + sum(stg$mStorageFixom(stg, region, year), pStorageFixom(stg, region, year) * vStorageCap(stg, region, year))
...
*!        + sum(stg$mStorageVarom(stg, region, year), vStorageVarom(stg, region, year)$mStorageVarom(stg, region, year))
        + sum((stg, comm)$mStorageComm(stg, comm),
             sum(slice$mCommSlice(comm, slice),
                 pStorageCostInp(stg, region, year, slice)
                    * pSliceWeight(year, slice)
                    * vStorageInp(stg, comm, region, year, slice)
...
*!        + sum(trade$mTradeEac(trade, region, year), vTradeEac(trade, region, year)$mTradeEac(trade, region, year))
        + sum((trade, yearp)$(mTradeNew(trade, yearp) and  ordYear(year) >= ordYear(yearp) and
              (ordYear(year) < pTradeOlife(trade) + ordYear(yearp) or mTradeOlifeInf(trade))),
                pTradeEac(trade, region, yearp) *
                vTradeNewCap(trade, yearp)
          )
* IR trade fixed O&M costs
        + sum(trade$mTradeSpan(trade, year),
              pTradeFixom(trade, region, year) * vTradeCap(trade, year)$mTradeSpan(trade, year))
* IR trade activity costs
*        + vTradeIrCost(region, year)$mvTradeIrCost(region, year)
...
$include data.gms
*mvSupCost(sup, region, year) = NO;
*mTechEac(tech, region, year) = NO;
*mTechFixom(tech, region, year) = NO;
*mTechVarom(tech, region, year) = NO;
*mStorageEac(stg, region, year) = NO;
*mStorageFixom(stg, region, year) = NO;
*mStorageVarom(stg, region, year) = NO;
*mTradeEac(trade, region, year) = NO;
*mTradeFixom(trade, region, year) = NO;
*mTradeIrCost(trade, region, year, slice) = NO;
*mImportIrCost(trade, region, year) = NO;
*mExportIrCost(trade, region, year) = NO;
*mExportRowCost(expp, region, year) = NO;
*mImportRowCost(imp, region, year) = NO;
