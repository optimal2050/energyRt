...
eqCost(region, year)$mvTotalCost(region, year)..
        ...
        =e=
* supply costs
        + sum(sup$mvSupCost(sup, region, year), pPeriodLen(year) * vSupCost(sup, region, year))
* technology investments
        + sum(tech$mTechEac(tech, region, year), pPeriodLen(year) * vTechEac(tech, region, year))
...
* technology O&M costs
        + sum(tech$mTechOMCost(tech, region, year), pPeriodLen(year) * vTechOMCost(tech, region, year))
* storage investments
        + sum(stg$mStorageEac(stg, region, year), pPeriodLen(year) * vStorageEac(stg, region, year))
* storage O&M costs
        + sum(stg$mStorageOMCost(stg, region, year), pPeriodLen(year) * vStorageOMCost(stg, region, year))
* trade costs (total)
        + vTradeCost(region, year)$mvTradeCost(region, year)
* commodity taxes
        + sum(comm$mTaxCost(comm, region, year), pPeriodLen(year) * vTaxCost(comm, region, year))
* commodity subsidies
        - sum(comm$mSubCost(comm, region, year), pPeriodLen(year) * vSubsCost(comm, region, year))
* custom user costs
...
* "dummy" import costs for debugging
        + sum((comm, slice)$mDummyImport(comm, region, year, slice),
              ...
              * pSliceWeight(year, slice) * pPeriodLen(year)
...
*           pDiscountFactorMileStone(region, year) *
            vTotalCost(region, year) * pDiscountFactor(region, year));
