...
STG_BTR <- newStorage(
  ...
  invcost   = list(
    out.invcost = 12144,                  # EUR/MW  -- the inverter
    stg.invcost = 8081                    # EUR/MWh -- the cells
  ),
...
STG_H2 <- newStorage(
  ...
  input     = list(comm = "ELC", unit = "MWh"),   # electrolyser
  output    = list(comm = "ELC", unit = "MWh"),   # fuel cell
  storage   = list(comm = "H2",  unit = "MWh"),   # the cavern
  invcost   = list(
    inp.invcost = 226174,                 # the electrolyser
    stg.invcost = 112                     # the cavern
  ),
  seff      = data.frame(
    inpeff  = 0.70,
    outeff  = 0.50
...
#> Solver directory:  scenarios/fy-year_d365-h24-subset-1day-per-month/runs/glpk/solver 
#> Writing files: 0.57s
#> Starting  GLPK 
#> 0.17s
#> Reading solution: 0.19s
scen_day <- solve_model(mod_day, name = "fy_day")
#> Solver directory:  scenarios/fy-day_d365-h24-subset-1day-per-month/runs/glpk/solver 
#> Writing files: 0.58s
#> Starting  GLPK 
#> 0.14s
#> Reading solution: 0.19s
...
STG_FLEX <- newStorage(
  ...
  output    = list(comm = "ELC", unit = "MWh"),
  storage   = list(comm = "ELC", unit = "MWh"),
  invcost   = list(out.invcost = 12, stg.invcost = 8),
  duration  = data.frame(
    duration.lo = 2,
    duration.up = 8
  ),
...
timeslice, so the rate has to be converted at the bound:
    ...
    vStorageInp[.,s] <= inp.af.up * cap2act * vStorageInpCap * pTimesliceShare[s]
...
calendar adequate. Discharge in a slice is bounded by three things:
    ...
    (1) out <= out.af.up · cap2act · outCap · share[s]
...
| `inp.af.lo` / `inp.af.up` / `inp.af.fx` | **charging**, against the charger |
| `out.af.lo` / `out.af.up` / `out.af.fx` | **discharging**, against the discharger |
...
STG_RES <- newStorage(
  ...
  commodity = "ELC",
  storage   = list(comm = "ELC", unit = "MWh"),
  invcost   = list(stg.invcost = 40),
  af        = data.frame(
    timeslice = c(NA, "d046_h18"),
    af.lo     = c(0.15, NA),              # never draw below 15% full
    inp.af.up   = c(NA, 0)                  # no charging in that slice
  ),
  vintage   = data.frame(olife = 40L)
)
STG_RES@af
#>   vintage cluster region year timeslice af.lo af.up af.fx inp.af.up inp.af.fx
#> 1    <NA>    <NA>   <NA>   NA      <NA>  0.15    NA    NA        NA        NA
#> 2    <NA>    <NA>   <NA>   NA  d046_h18    NA    NA    NA         0        NA
#>   inp.af.lo out.af.up out.af.fx out.af.lo
#> 1        NA        NA        NA        NA
#> 2        NA        NA        NA        NA
```
...
`@capacity` holds every capacity the storage has, in the same shape a
`technology` uses — existing `stock`, bounds on total capacity
(`cap.*`), on new builds (`ncap.*`) and on retirement (`ret.*`) — but
**once per part**, under a prefix naming the part it belongs to:

| prefix | the part       | measured in |
|--------|----------------|-------------|
| `out.` | the discharger | power       |
| `inp.` | the charger    | power       |
| `stg.` | the reservoir  | energy      |

So `out.cap.up` bounds the discharger, `inp.cap.fx` fixes the charger,
and `stg.stock` is the reservoir you already own. A charger rated
differently from the discharger — an EV drawing 7 kW but delivering 100
kW to the motor — is `inp.cap.fx = 7` beside `out.cap.fx = 100`.

The role slots `@input` / `@output` / `@storage` carry no capacity at
all. They declare *what* each side handles — `comm`, `unit`, and
`cap2act` on the two power sides — and nothing that varies by vintage,
region or year. Writing a capacity there is an error that names the
...
STG_FLEET <- newStorage(
  ...
  capacity  = data.frame(
    region  = "R1",
    year    = 2020L,
    out.stock   = 120,                        # already built
    out.ncap.up = 50                          # at most 50 more in this year
  ),
  vintage   = data.frame(olife = 15L)
)
STG_FLEET@capacity
#>   vintage cluster region year out.stock out.cap.lo out.cap.up out.cap.fx
#> 1    <NA>    <NA>     R1 2020       120         NA         NA         NA
#>   out.ncap.lo out.ncap.up out.ncap.fx out.ret.lo out.ret.up out.ret.fx
#> 1          NA          50          NA         NA         NA         NA
#>   inp.stock inp.cap.lo inp.cap.up inp.cap.fx inp.ncap.lo inp.ncap.up
#> 1        NA         NA         NA         NA          NA          NA
#>   inp.ncap.fx inp.ret.lo inp.ret.up inp.ret.fx stg.stock stg.cap.lo stg.cap.up
#> 1          NA         NA         NA         NA        NA         NA         NA
#>   stg.cap.fx stg.ncap.lo stg.ncap.up stg.ncap.fx stg.ret.lo stg.ret.up
#> 1         NA          NA          NA          NA         NA         NA
#>   stg.ret.fx
#> 1         NA
```
...
|----|----|----|
| `@invcost` | new capacity | `<part>.invcost`, `.wacc`, `.payback`, `.eac`, `.retcost` |
| `@fixom` | installed capacity, per year | `<part>.fixom` |
| `@varom` | flows and level, per timeslice | `inpcost`, `outcost`, `stgcost` |

`@invcost` and `@fixom` take the same `out.` / `inp.` / `stg.` prefixes
as `@capacity`, so each part is priced in its own units: `out.invcost`
per MW of discharger, `stg.invcost` per MWh of reservoir — which is
exactly what the battery at the top of this article does. `@varom`
instead splits by what is charged: `inpcost` per unit charged, `outcost`
per unit discharged, `stgcost` per unit held.
...
STG_PRICED <- newStorage(
  ...
  commodity = "ELC",
  duration  = 6,
  invcost   = data.frame(out.invcost = 8081), # EUR/MWh on new capacity
  fixom     = data.frame(out.fixom = 5),      # EUR/MW/yr on the whole store
  varom     = data.frame(
    inpcost = 0.5,
    outcost = 0.5
  ),
...
STG_HYD_W <- newStorage(
  ...
  weather   = data.frame(
    ...
    inp.waf.fx = 1                          # inflow drives charging
  ),
  vintage   = data.frame(olife = 60L)
)
```
...
STG_VINT <- newStorage(
  ...
  storage   = list(comm = "ELC", unit = "MWh"),
  # the reservoir is what gets cheaper, so the per-vintage price is `stg.`
  invcost   = data.frame(
    vintage     = c("v2020", "v2050"),
    stg.invcost = c(8081, 3200)           # cells get cheaper
  )
)
STG_VINT@vintage
...
STG_VINT@invcost[, c("vintage", "stg.invcost")]
#>   vintage stg.invcost
#> 1   v2020        8081
#> 2   v2050        3200
```
...
#> Solver directory:  scenarios/solar-battery_d365-h24-subset-1day-per-month/runs/glpk/solver 
#> Writing files: 0.42s
#> Starting  GLPK 
#> 0.26s
#> Reading solution: 0.19s
...
STG_BTR_VRE <- newStorage(
  ...
  output    = list(comm = "ELC", unit = "MWh"),
  storage   = list(comm = "ELC", unit = "MWh"),
  # power priced per MW on the discharger, energy per MWh on the reservoir
