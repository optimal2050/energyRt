...
The energyRt optimization model — about 150 predefined equations over 95
...
Two further routes send the solve elsewhere, reusing those formulations
rather than adding a fifth: **NEOS** runs CPLEX and friends on a free
remote server (no local licence), and **`multimod_cloud`**
(experimental) writes the problem as MPS and solves it on a cloud GPU
for models too large for a laptop.
